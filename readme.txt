=== FP Distributor Media Kit ===

Contributors: franpass87
Tags: media kit, distributor, download, private area, user approval
Requires at least: 6.0
Tested up to: 6.7
Stable tag: 1.21.17
Requires PHP: 8.0
License: Proprietary
License URI: https://francescopasseri.com

Area riservata per distributori: registrazione, approvazione admin, download asset protetti.

== Description ==

Plugin per creare un'area riservata "Media Kit" dove i distributori approvati possono scaricare asset (immagini, PDF, video).

* Registrazione frontend con approvazione admin obbligatoria
* Login/Logout dedicato
* Asset organizzati per categoria e lingua
* Download sicuro (nessun link diretto ai file)
* Tracking download
* Notifica email a distributori approvati

== Installation ==

1. Carica la cartella in wp-content/plugins/
2. Attiva il plugin
3. Configura le pagine in FP Media Kit → Impostazioni
4. Inserisci gli shortcode nelle pagine create

Shortcode: [fp_dmk_register], [fp_dmk_register_it], [fp_dmk_register_en], [fp_dmk_login], [fp_dmk_login_it], [fp_dmk_login_en], [fp_dmk_media_kit], [fp_dmk_media_kit_it], [fp_dmk_media_kit_en] (senza suffisso = lingua pagina; _it / _en forzano l’interfaccia)

== Changelog ==

= 1.21.17 =
* Fixed: login/media-kit UI lingua — priorità alla lingua effettiva della richiesta (`determine_locale` della pagina) per evitare inglese su URL italiani con WPML.

= 1.21.16 =
* Improved: rilevamento lingua pagina per **WPML** — `wpml_post_language_details`, `SitePress::get_current_language()`, filtro `wpml_current_language`, costante `ICL_LANGUAGE_CODE`; TranslatePress non interferisce se WPML è attivo.

= 1.21.15 =
* Fixed: errore di sintassi in `ShortcodeUiLang` (release 1.21.14); stessi filtri gettext di 1.21.14 ripristinati correttamente.

= 1.21.14 =
* Fixed: frontend — con locale sito in inglese, un `.mo` del dominio `fp-dmk` non sovrascrive più i msgid italiani quando l’interfaccia non è in modalità inglese (`*_en`).

= 1.21.13 =
* Fixed: rilevamento lingua — Polylang prima di TranslatePress (evita interfaccia EN su pagine IT); `redirect_to` senza doppia codifica URL.

= 1.21.12 =
* Fixed: rilevamento lingua pagina — niente più inglese forzato dal solo `en_US` di WordPress; Polylang (post, corrente, cookie), TranslatePress, WPML e URL `/en/` restano i segnali validi.

= 1.21.11 =
* Added: shortcode senza suffisso — interfaccia IT/EN in base alla lingua pagina (Polylang/WPML, URL `/en/…`, locale).

= 1.21.10 =
* Fixed: Media Kit — catalogo asset comune su IT e EN; gli shortcode `*_en` localizzano solo etichette e testi UI.

= 1.21.9 =
* Changed: Media Kit — pagine IT/EN mostrano per default solo asset con lingua IT o EN; «Tutte le lingue» resta disponibile nel filtro.

= 1.21.8 =
* Changed: Media Kit — con una sola categoria nella cartella, niente accordion annidato ridondante.

= 1.21.7 =
* Changed: Media Kit — blocco «Materiali generali» aperto di default; altre cartelle e categorie chiuse.

= 1.21.6 =
* Fixed: filtri Media Kit — cartella/categoria/lingua allineati all’elenco asset e dropdown popolati correttamente.

= 1.21.5 =
* Added: tipi di accesso — etichette IT/EN in impostazioni e in registrazione EN.

= 1.21.4 =
* Fixed: impostazioni Email — toggle in elenco dedicato e switch allineati.

= 1.21.3 =
* Added: email notifiche amministratore — più destinatari separati da virgola.

= 1.21.2 =
* Fixed: registrazione EN — etichette tipi di accesso (Distributore/Giornalista) tradotte in interfaccia inglese.
* Fixed: CTA frontend — testo leggibile su hover/focus anche per link Download con bottoni primari scuri.

= 1.21.1 =
* Changed: Media Kit — accordion cartelle/categorie chiusi al caricamento pagina.

= 1.21.0 =
* Added: shortcode IT/EN per registrazione, login e Media Kit (`*_it`, `*_en`; default = italiano).

= 1.20.11 =
* Fixed: filtro Cartella Media Kit — ID cartelle da SQL (come Categorie v1.20.10).

= 1.20.10 =
* Fixed: filtro Categoria Media Kit — opzioni select da SQL (term_relationships) al posto di get_terms(object_ids).

= 1.20.9 =
* Added: Media Kit — cartelle e categorie in accordion (da v1.21.1 tutti chiusi al caricamento).

= 1.20.8 =
* Fixed: colonna Categoria duplicata in elenco asset; lettura categorie da SQL; checkbox categoria nel metabox Dettagli asset con salvataggio wp_set_object_terms.

= 1.20.7 =
* Fixed: lettura categorie asset affidabile (wp_get_post_terms + fallback SQL); tassonomia in REST per editor a blocchi.
* Changed: Media Kit, report e audience usano AssetManager::get_asset_category_terms.

= 1.20.6 =
* Changed: Media Kit frontend — asset in elenco a righe (non più griglia card); ZIP bulk invariato.

= 1.20.5 =
* Fixed: card Media Kit — se manca la categoria tassonomia ma c’è la cartella, mostra percorso cartella invece di «Altro».

= 1.20.4 =
* Changed: card Media Kit — area superiore con categorie (chip) al posto delle anteprime file/immagine.

= 1.20.3 =
* Fixed: filtri Media Kit — opzioni cartella/categoria da get_terms(object_ids); slug categoria da GET coerente con audience.

= 1.20.2 =
* Changed: admin — lista asset, filtri e tassonomie a larghezza piena; impostazioni fino a 1600px.

= 1.20.1 =
* Fixed: rimozione filtro SQL ricerca Media Kit garantita con try/finally (evita WHERE residuo su altre query).

= 1.20.0 =
* Added: Media Kit — ricerca (fp_dmk_q), ordinamento (fp_dmk_sort), conteggio risultati, anteprime card, download ZIP multiplo (max 25) con log per file; widget bacheca attività; login/registrazione — mostra/nascondi password, indicatore forza password, link sito e privacy.
* Note: export CSV log e report già disponibili da Log download e Report.

= 1.19.6 =
* Changed: rifinitura interfaccia frontend (tipografia, pannelli login/registrazione, card asset, select, focus e messaggi).

= 1.19.5 =
* Fixed: dropdown cartelle e categorie nel Media Kit popolati dagli asset visibili (evita elenco vuoto con hide_empty / conteggi WP).

= 1.19.4 =
* Changed: interfaccia shortcode Media Kit (header, filtri in card con etichette, pulsante primario, link Reimposta); blocco senza cartella rinominato «Materiali generali» con testo esplicativo.

= 1.19.3 =
* Fixed: lingua predefinita sul caricamento multiplo salvata per utente (user meta + AJAX).

= 1.19.2 =
* Added: elenco file in coda per la cartella selezionata (vista tipo client FTP), aggiornamento live; click per saltare alla riga nella tabella.

= 1.19.1 =
* Fixed: colonna Cartelle sul caricamento multiplo senza overflow orizzontale (form Nuova cartella).

= 1.19.0 =
* Changed: menu FP Media Kit con hub su Caricamento multiplo; voce «Elenco asset» per la lista CPT; rimosse voci ridondanti (Categorie, Cartelle, Aggiungi nuovo).
* Added: creazione categorie da Caricamento multiplo (Nuova categoria); filtro `fp_dmk_bulk_first_admin_menu` per disattivare l’hub.

= 1.8.0 =
* Added: cartelle gerarchiche per ordinare gli asset (tassonomia fp_dmk_folder), select in modifica asset, filtro e raggruppamento nel Media Kit.

= 1.7.1 =
* Fix: audience attiva solo se esiste almeno un segmento (evita errore registrazione con lista vuota).

= 1.7.0 =
* Added: tipi di accesso (audience) — segmenti in impostazioni, limiti per categoria asset, campo in registrazione, colonna in lista utenti, tipo in email admin.

= 1.6.1 =
* Fix: utenti con ruolo Gestore Media Kit (`manage_fp_dmk`) trattati come staff in `is_approved` (accesso frontend / download coerente con admin).

= 1.6.0 =
* Added: ruolo "FP Media Kit Manager" (fp_dmk_manager) per gestire il plugin senza essere Administrator.
* Changed: capability dedicate per asset e categorie (Administrator aggiornato automaticamente; ruoli solo con edit_posts non gestiscono più gli asset).

= 1.5.8 =
* Fix: capability admin `manage_fp_dmk` ripristinata al load se assente (evita accesso negato dopo deploy senza riattivazione).

= 1.5.7 =
* Changed: Email notifiche — branding FP Mail SMTP su HTML frammento (`fp_fpmail_brand_html`).

= 1.5.6 =
* Fix intestazioni tabella: testo e icone bianchi su sfondo viola (contrasto)

= 1.5.5 =
* Fix pagine Media Kit Asset e Categorie: design system FP (card, thead viola, form, postbox)

= 1.5.4 =
* Fix grafica sezioni/box: card, tabella, input secondo design system FP

= 1.5.3 =
* Pagina Lista utenti: distributori con filtro e azioni Approva/Revoca

= 1.5.2 =
* Compatibilità FP Mail SMTP: opzione mittente in Impostazioni

= 1.5.1 =
* Banner grafica FP unificato su lista asset, aggiungi/modifica asset, categorie

= 1.5.0 =
* Pagina Report: statistiche generali, attività per utente e per asset
* KPI, Top 10 asset/utenti, export CSV

= 1.4.2 =
* Grafica admin unificata al design system FP
* Enqueue CSS centralizzato con pattern strpos + post_type/taxonomy
* Menu position 56.13, admin body class fpdmk-admin-shell

= 1.4.1 =
* Eventi fp_tracking_event per integrazione FP-Marketing-Tracking-Layer
* Fix enqueue admin CSS su menu sotto parent

= 1.4.0 =
* Validazione password forte (maiuscola, minuscola, numero)
* Link incrociati Login/Registrazione
* Filtri Media Kit (categoria, lingua)
* Pagina Log download con export CSV
* Cron pulizia log automatica
* Accessibilità migliorata

= 1.3.0 =
* Checkbox "Ricordami" nel form login
* Indicatore caricamento su submit form
* Messaggio Media Kit vuoto migliorato
* Sanitizzazione fp_dmk_registered

= 1.2.0 =
* Bypass approvazione per admin/editor
* Link "Password dimenticata?" nel form login
* Supporto username nel login (oltre all'email)
* Token CSS design system allineati
* Sanitizzazione $_GET nelle notice admin

= 1.1.0 =
* Aggiunta sezione Aspetto: personalizza colori bottoni, sfondi card, bordi

= 1.0.0 =
* Release iniziale
